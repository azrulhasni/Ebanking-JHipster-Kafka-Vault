...
class TransactionKafkaResourceIT {
    ...
    void setup() {
        ...
        TransactionKafkaResource kafkaResource = new TransactionKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/transaction-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/transaction-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
