...
import com.azrul.ebanking.depositaccount.config.KafkaProperties;
...
class DepositAccountKafkaResourceIT {
    ...
    void setup() {
        ...
        DepositAccountKafkaResource kafkaResource = new DepositAccountKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/deposit-account-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/deposit-account-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
